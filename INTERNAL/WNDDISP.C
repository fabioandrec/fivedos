...
CLIPPER VMBoxDispl() //  cVPort, nBoxRect, nClrIzqArr, nClrDchaAbjo [, cBoxString ]
{
   PCLIPVAR  vPort  = _param( 1, CHARACTER );
   BOOL bNeedUnLock = _VSTRLOCK( vPort );
   LPSCRPORT pPort  = (LPSCRPORT ) _VSTRX( vPort );
   ...
   WORD w, wT;
   LPBYTE pBox      = IF( _parclen( 5 ) >= 8, _parc( 5 ), 0 );
   ...
   if( pBox )
   {
       bBorderUpLeft  = pBox[ 0 ];
       bBorderTop     = pBox[ 1 ];
       bBorderUpRight = pBox[ 2 ];
       bBorderRight   = pBox[ 3 ];
       bCornerDRight  = pBox[ 4 ];
       bBorderBottom  = pBox[ 5 ];
       bCornerDLeft   = pBox[ 6 ];
       bBorderLeft    = pBox[ 7 ];
   }
   else
   {
       bBorderUpLeft  = BORDERUPLEFT;
       bBorderTop     = BORDERTOP;
       bBorderUpRight = BORDERUPRIGHT;
       bBorderRight   = BORDERRIGHT;
       bCornerDRight  = CORNERDRIGHT;
       bBorderBottom  = BORDERBOTTOM;
       bCornerDLeft   = CORNERDLEFT;
       bBorderLeft    = BORDERLEFT;
   }
   ...
   VPWriteAt( pPort, pRect->iTop, pRect->iLeft, &bBorderUpLeft,
              wClrUpLeft, 1 );
   VPSayRep( pPort, pRect->iTop, pRect->iLeft + 1, bBorderTop,
   ...
   VPWriteAt( pPort, wT, pRect->iLeft, &bCornerDLeft,
              wClrUpLeft, 1 );
   VPSayRep( pPort, wT, pRect->iLeft + 1, bBorderBottom,
...
//     WORD wChecked   = _parl( 6 );
    BYTE bBackClr   = ( (BYTE) wColor ) & 0xF0;
    ...
    if( wLen > 2 )
    {
        ...
        bTemp[ 0 ] = IF( _parl( 7 ), MENUITEMOPEN, ' ' );
        VPSay( pPort, 0, iRight - 1, bTemp, wColor );
        VPSayHot( pPort, 0, 1, _parc( 2 ), wColor, _parni( 5 ) );
    }
    else
        VPSayHot( pPort, 0, 0, _parc( 2 ), wColor, _parni( 5 ) );
